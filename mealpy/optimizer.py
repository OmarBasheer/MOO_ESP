...
class Optimizer:
    ...
    def get_target_wrapper(self, position, a, b, s=None):
        """
        Args:
        ...
        Returns:
            [fitness, [obj1, obj2,...]]
        """
        eb = getBill(position, s)
        par = getPAR(position, s)
        wtr = getWTR(position, s)
        cpr = getCPR(position, s)
        objs = self.problem.fit_func(eb, par, wtr, cpr, a, b)
    ...
    def create_population(self, pop_size=None):
        ...
        if self.mode == "thread":
            with parallel.ThreadPoolExecutor() as executor:
                list_executors = [executor.submit(self.create_solution, self.problem.lb, self.problem.ub, a, b, s) for _ in range(pop_size)]
            ...
            pop = [self.create_solution(self.problem.lb, self.problem.ub, self.problem.loc, a, b, s) for _ in range(0, pop_size)]
        return pop

    def update_target_wrapper_population(self, pop, a, b, s=None):
        """
        ...
        if self.mode == "thread":
            with parallel.ThreadPoolExecutor() as executor:
                list_results = executor.map(self.get_target_wrapper, pos_list, a, b, s)  # Return result not the future object
            ...
            for idx, pos in enumerate(pos_list):
                pop[idx][self.ID_TAR] = self.get_target_wrapper(pos, a, b, s)
        return pop
